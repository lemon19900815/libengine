...
bool NetCore::initialize(int maxeventsize)
{
  if (!ae_) {
    ae_ = aeEventLoop::create(maxeventsize);
    assert(ae_);

    sessions_ = new Session::WPTR[maxeventsize];

    ae_->acceptProc = std::bind(&NetCore::onAccept, this, std::placeholders::_1);
    ae_->closeProc = std::bind(&NetCore::onClose, this, std::placeholders::_1);
...
*/
void NetCore::onAccept(Session::PTR session)
{
  // array operate don't need use mutex
...
*/
void NetCore::onClose(Session::PTR session)
{
  // don't need to close this session, we used weak_ptr to handle it
  /*mutex_.lock();
  sessions_[session->fd()] = session;
  mutex_.unlock();*/
}
